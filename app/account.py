...
def app():
    ...
    if st.session_state['authenticated']:
        st.write(f"Welcome, {st.session_state['username']} :wave:")
        ...
        if st.button('Log out'):
            ...
            st.session_state['username'] = ""
            st.session_state['useremail'] = ""
            st.write("You have successfully logged out. Don’t forget to sign in next time!")
        ...
        if choice == 'Sign up':
            ...
            if st.button('Create my account'):
                ...
                if user:
                    ...
                    st.balloons()
                    st.success("!Account Created Successfully! Enjoy your experience.")
        ...
        # Lógica para Login
            ...
            if st.button('Login'):
                ...
                if userinfo:
                    ...
                    st.session_state['authenticated'] = True
                    st.balloons()
                    st.success("Successful Login: Enjoy your experience!")
# Función de registro
def sign_up_with_email_and_password(email, password, username=None, return_secure_token=True):
    try:
        st.write("Attempting to Register User with Email.:", email)
        ...
        if "error" in  response_json:
            st.warning("Registration Error: " +  response_json['error']['message'])
            return None
        else:
            st.success("User Registered Successfully.")
            return response_json.get('email')  # Devolver correo registrado si tiene éxito
    except Exception as e:
        st.warning(f'Registration Error: {e}')
